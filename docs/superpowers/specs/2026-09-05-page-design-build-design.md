...
| Site map change | `/team` (existing URL) is repurposed to Elizabeth's professional profile — a route being repurposed with new content isn't a URL retirement, so no redirect is needed; the URL keeps resolving, just with different content, and Google re-crawls and updates its understanding over time like any site relaunch. Adds one new route, `/reviews`, for the 27 testimonials — a brand-new page with no prior URL history to preserve |
| `/about` vs `/team` | `/about` = company story & mission (women-owned, inspiring women into trades). `/team` = Elizabeth's straightforward professional bio/profile. Distinct purposes, not merged |
...
| Home | Hero (real photo + headline + women-owned messaging + CTA), services preview grid, a short "meet Elizabeth" teaser linking to `/team`, 3-4 featured testimonials, closing CTA |
| About | Company story & mission content only |
| Team (`/team`, repurposed) | Elizabeth's profile: role, background (SCAD, M.Arch, patent holder). No headshot photo exists yet — graceful text-only layout, not a broken image slot |
...
| Reviews (`/reviews`, new) | All 27 testimonials in a clean grid — full list, no pagination or curation (a 5-star-reviewed business's full review volume is a real asset) |
...
- Every one of the 12 routes (11 existing + `/reviews`) renders real content with the new
  design, not placeholder `<h1>` text
- `/team` shows Elizabeth's profile, not the old reviews placeholder content
...
- Header nav includes the new `/reviews` link
